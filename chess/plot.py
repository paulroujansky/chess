...
def show_board(board, cmap=CMAP, piece=None, moves=None):
    ...
    if piece is not None:
        ...
        if moves is not None and len(moves) > 0:
            moves_idx = [coords_to_np(move.get_new_coords(
                piece.coords)) for move in moves]
    ...
    sns.heatmap(background, cmap=cmap, center=center,
                # annot=board, annot_kws={'fontsize': 14, 'fontweight': 'bold'},  # noqa: E501
                fmt='s', square=True, cbar=False, linewidths=1,
        ...
        for j in range(8):
            if board[i, j]:
                ...
                path_effects = [pe.withStroke(
                    linewidth=3, foreground='black')] if col == 'white' else []
                if sym != ' ':
                    ax.text(j + .5, i + .5, sym,
